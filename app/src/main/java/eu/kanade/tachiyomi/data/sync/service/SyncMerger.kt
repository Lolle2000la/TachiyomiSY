...
/**
 * Merges two [SyncData] documents into one.
 *
 * Extracted from [SyncService]: the merge rules are pure functions of the two documents plus
 * [SyncPreferences], so keeping them here lets them be unit tested without an Android context.
 * The two rules the tests exercise directly ([mergeMangaLists] and [mergeChapters]) are therefore
 * `internal` rather than `private`.
 *
 * @param syncPreferences The preferences holding the sync options, the device id and the last sync
 * timestamp, which is what distinguishes a newly added entry from one the other side deleted.
 */
class SyncMerger(
